...
ChatLogic::~ChatLogic()
{
    ...
    // no need to delete nodes, since it's a vector of unique pointers that will be exist on the stack as long as it's in scope

    // edges no longer owned by ChatLogic
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            (*parentNode)->AddEdgeToChildNode(std::move(edge)); //graph node owns outgoing/child  edge
                        }
    ...
    // communication handle between GUI and ChatBot
